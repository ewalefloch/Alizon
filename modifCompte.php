      ...
      <div class="block">
        ...
        <div class="panels">
          ...
          <div class="panel" id="one-panel">
            ...
            <?php
              $idclient = 0; //id du client a modifier
              if(isset($_POST["idclient"])){
                $_SESSION["modifidclient"] = $_POST["idclient"];
              }
              
              if(isset($_SESSION["modifidclient"])){
                $idclient = $_SESSION["modifidclient"];
              }else if(isset($_SESSION["idclient"])){
                $idclient = $_SESSION["idclient"];
              }else{
                //erreur pas idclient a modifier
                echo "erreur";
              }
              ...
              if($idclient){
                if(isset($_POST['nom'])){
                  setNom($idclient, $_POST['nom']); 
                }
                if(isset($_POST['prenom'])){
                  setPrenom($idclient, $_POST['prenom']);
                }
                if(isset($_POST['email'])){
                  setEmail($idclient, $_POST['email']);
                }
                if(isset($_POST['telephone'])){
                  setTelephone($idclient, $_POST['telephone']);
                }
                if(isset($_POST['ddn'])){
                  setDateDeN($idclient, $_POST['ddn']);
                }

                if(isset($_POST['nomrue']) && isset($_POST['numrue']) && isset($_POST['codepostal']) && isset($_POST['ville'])){
                  setAdresse($idclient, $_POST['codepostal'], $_POST['ville'], $_POST['nomrue'], $_POST['numrue'], $_POST['infos']);
                }
                if( isset($_POST['infocompte'])){
                  echo '<div id="popInsCon"><p>Informations mise à jour </p></div>';
                }
              }
            ?>
            <h4>Mon compte</h4>
            <form method="post">
              <div class="hcontainer">
                <div class="champ">
                  ...
                  <input type="text" name="prenom" value="<?php echo getPrenom($idclient) ?>" required/>
                </div>
      
                <div class="champ">
                  <label>Nom</label><br/>
                  <input type="text" name="nom" value="<?php echo getNom($idclient) ?>"required/>
                </div>
              </div>
              ...
              <div class="hcontainer">
                <div class="champ">
                  ...
                  <input type="email" name="email" value="<?php echo getEmail($idclient) ?>" required/><br/>
                </div>

                <div class="champ">
                  <label>Téléphone</label><br/>
                  <input type="tel" name="telephone" pattern="[0-9]{10}" value="<?php echo getTelephone($idclient) ?>" required/><br/>
                </div>
              </div>
              <div class="champ">
                <label>Date de naissance</label><br/>
                <input type="date" name="ddn" value="<?php echo getDateDeN($idclient) ?>" required/><br/>
              </div>
            
              <h4>Adresse de facturation</h4>
              <?php $adresse = getAdresse($idclient)->fetchAll()[0];?>
              ...
              <div class="hcontainer containervalide">
                ...
                if(isset($_SERVER['modifidclient'])){
                  echo "<a href='monProfil.php'><p class='retour'>< retour<p></a>";
                }else{
                  echo "<a href='listerComptes.php'><p class='retour'>< retour<p></a>";
                }
                ?>
            ...
            <?php 
              if(isset($idclient)){
                if(isset($_POST['actuelmdp']) && isset($_POST['nouveaumdp']) && isset($_POST['nouveaumdpconf'])){
                  if(chiffrement_mdp($_POST['actuelmdp'])== getMDP($idclient) ){
                    if($_POST['nouveaumdp'] == $_POST['nouveaumdpconf']){
                      if($_POST['nouveaumdp'] != ""){
                        if($_POST['actuelmdp'] != $_POST['nouveaumdp']){
                          setMDP($idclient, $_POST['nouveaumdp']);
