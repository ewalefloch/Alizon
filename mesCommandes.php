...
<h2>MES COMMANDES</h2>
        ...
        <?php
            try {
                $comm = $dbh->prepare("SELECT * from {$prefix}_commande c inner join {$prefix}_client a on c.idclient=a.idclient  where c.idclient=$idclient ORDER BY idcommande DESC");
                $comm->execute();
                $compteur=0;
                if ($comm->rowCount()==0){
                    ...
                    echo '<div class="unecomm"><article class="commande_princ" >
                        ...
                        // if($return == 0){
                          ...
                          ?>
                            ...
                            <input type="hidden" name="numComRecept" value=<?php echo $key['idcommande']?>></input>
                            <input class="valide" type="submit" name="sub" value="Valider la reception"></input>
                            </form>
                          <?php 
                          }
                        // }
