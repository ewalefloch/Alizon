...
void MAJEtatCommande(commande* com){
    if(time(0) > com->dateLivraison){
        com->etat = Livre;
    }
    if(time(0) > com->dateFinVersSiteLocal){
        com->etat = Livre;
    }
    if(time(0) > com->dateFinTransportRegion){
...
int main(int argc, char* argv[]){
    //option en plus chemin du fichier client
    //option de ne pas log / chemin du fichier log
    ...
    File listeCommandeEnCours;
    File fileCommandeEnAttente;
    File fileCommandeTermine;

    initFile(&listeCommandeEnCours);
    initFile(&fileCommandeEnAttente);
    initFile(&fileCommandeTermine);
            ...
            if(read(cnx, proto, sizeof(protocol)) > 0){
                ...
                if(proto->version != PROTOCOL_VERSION){
                    ...
                    case EnvoieCommande :{
                        ...
                        //verif le token
                        commande* com  = getCommande(&listeCommandeEnCours, idcommande);
                        if(!tokenValide(ArbreTokens, id_token)){//si le token n'existe pas alors
                        ...
                        else if(tailleFile(&listeCommandeEnCours) >= tailleMaxFileCommande){
                            coderet = FileDeLivraisonPleine; //code retour;
                            printf("erreur file de livraison pleine\n"); //envoie -1 pour dire au client que sont token n'est pas ou plus valide
                        }
                        else{
                            //insere dans la liste des commandes
                            commande com;
                            com.etat = PrisEnCharge;
                            ...
                            insereFile(&listeCommandeEnCours, com);
                    ...
                    case DemandeInfo :{
                        ...
                        commande* com  = getCommande(&listeCommandeEnCours, idcommande);
                        commande* comT = getCommande(&fileCommandeTermine, idcommande);
                    ...
                    case AccuseDeReception:{
                        ...
                        if(!tokenValide(ArbreTokens, id_token)){//si le token et la commande existe alors
                            ...
                            //retire la commande de la file
                            commande com = retireCommande(&listeCommandeEnCours);
                            insereFile(&fileCommandeTermine, com);
                            coderet = Bon; //code retour;
                            write(cnx, &coderet, sizeof(coderet));
                    ...
                    case DemandeInfoListe:{
                        int id_token = 0;
                        int taille = tailleFile(&listeCommandeEnCours);
                        read(cnx, &id_token, sizeof(id_token));
